  ...
  static Future<void> generateAssessmentReport(
    ...
    pdf.addPage(
      pw.Page(
        ...
        theme: pw.ThemeData.withFont(base: font, bold: boldFont), // تعيين الخطوط
          ...
          return pw.Directionality(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                pw.Text('بيانات المقيم', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: PdfColors.blue800)),
                pw.SizedBox(height: 10),
                pw.Row(
                  children: [
                ...
                pw.Text('تاريخ التقييم: ${DateTime.now().toString().split(' ')[0]}'),
                pw.SizedBox(height: 20),
                
                // نتائج التقييم (Assessment Results)
                pw.Text('نتائج التقييم: ${tool.name}', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 10),
                ...
                pw.Container(
                  ...
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: [
                      pw.Text('الملخص التنفيذي:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.blue800)),
                      pw.SizedBox(height: 8),
                      pw.Text('تم إجراء التقييم بنجاح. حالة المقيم الحالية: ${resident.healthStatus == 'critical' ? 'حرجة' : resident.healthStatus == 'monitoring' ? 'تحتاج متابعة' : 'مستقرة'}.', style: const pw.TextStyle(fontSize: 11)),
                    ],
                  ),
                ),
                pw.SizedBox(height: 20),
                ...
                pw.Text('التفاصيل:', style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 10),
                ...
                ...questions.asMap().entries.map((entry) {
                  ...
                  final answerText = (q.options != null && answerIdx != -1 && answerIdx < q.options!.length) 
                      ? q.options![answerIdx] 
                      : 'لم تتم الإجابة';
                  ...
                  return pw.Padding(
                    ...
                    child: pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        pw.Text('${idx + 1}. ${q.text}', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: PdfColors.grey900)),
                        pw.Padding(
                          padding: const pw.EdgeInsets.only(right: 10, top: 4),
                          child: pw.Text('• الإجابة: $answerText', style: const pw.TextStyle(fontSize: 10, color: PdfColors.grey700)),
                        ),
                      ],
                    ),
                  );
