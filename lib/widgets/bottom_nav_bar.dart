...
class BottomNavBar extends StatelessWidget { // فئة شريط التنقل السفلي المخصص للمسن
  ...
  Widget build(BuildContext context) { // دالة بناء الواجهة
    ...
    final items = [ // قائمة عناصر شريط التنقل
      BottomNavItem(icon: Icons.home_rounded, label: 'الرئيسية', index: 0), // الرئيسية
      BottomNavItem(icon: Icons.medication_rounded, label: 'دواء', index: 1), // تنبيهات الأدوية
      const BottomNavItem(icon: Icons.circle, label: '', index: -1), // مساحة فارغة للكرة
      BottomNavItem(icon: Icons.people_rounded, label: 'أسرة', index: 2), // التواصل مع الأسرة
      BottomNavItem(icon: Icons.photo_rounded, label: 'ذكريات', index: 3), // ألبوم الصور
    ];
