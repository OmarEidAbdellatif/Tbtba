...
class AccessibilityDialog extends ConsumerWidget {
  ...
  Widget _buildScaleControl(AppRiverpod provider, WidgetRef ref) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.end,
      children: [
        Text('حجم الخط', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: hc ? Colors.white : const Color(0xFF1e293b))),
        const SizedBox(height: 16),
        Row(
          children: [
            Icon(Icons.text_fields_rounded, size: 20, color: hc ? Colors.white24 : const Color(0xFF94a3b8)),
            Expanded(
              child: Slider(
                ...
                max: 1.5,
                divisions: 7,
                activeColor: hc ? const Color(0xFF9FA8DA) : const Color(0xFF6C63FF),
                inactiveColor: hc ? Colors.white12 : const Color(0xFFe2e8f0),
                onChanged: (val) => ref.read(appRiverpod).updateFontScale(val),
              ),
            ),
            Icon(Icons.text_fields_rounded, size: 32, color: hc ? const Color(0xFF9FA8DA) : const Color(0xFF6C63FF)),
          ],
        ),
        Center(
          child: Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(color: hc ? const Color(0xFF252525) : const Color(0xFFf8fafc), borderRadius: BorderRadius.circular(16)),
            child: Text(
              'هذا مثال لحجم الخط الحالي',
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: hc ? Colors.white : const Color(0xFF1e293b)),
            ),
          ),
        ),
      ],
      ...
      children: [
        ...
        Expanded(
          child: FittedBox(
            ...
            child: Text('وضع التباين العالي (لضعف الرؤية)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: hc ? Colors.white : const Color(0xFF1e293b))),
          ),
        ),
      ],
    );
