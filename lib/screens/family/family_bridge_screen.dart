...
class _FamilyBridgeScreenState extends ConsumerState<FamilyBridgeScreen> with TickerProviderStateMixin {
  ...
  void _simulateUpload(String title, String type) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تمت إضافة ذكرى جديدة للحائط ❤️'), backgroundColor: Color(0xFF10b981)),
      );
    }
  }
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('تأكيد الإرسال', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1e293b))),
            const SizedBox(height: 8),
            Text('هل تريد إرسال ال$type الآن إلى الحاج محمود؟', style: const TextStyle(fontSize: 14, color: Color(0xFF64748b))),
            const SizedBox(height: 24),
            Row(
              children: [
                Expanded(
  ...
  Widget _buildUploadActions() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text('شارك لحظة جديدة', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1e293b))),
          const SizedBox(height: 16),
          Row(
            children: [
              _uploadBtn(Icons.mic_rounded, 'رسالة صوتية', const Color(0xFFa855f7), _isRecording ? _stopRecording : _startRecording, isRecording: _isRecording),
              const SizedBox(width: 12),
              _uploadBtn(Icons.image_rounded, 'صورة جديدة', const Color(0xFF0ea5e9), () => _showConfirmUpload('صورة')),
            ],
          ),
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text('${(_recordDuration ~/ 60).toString().padLeft(2, '0')}:${(_recordDuration % 60).toString().padLeft(2, '0')}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  const SizedBox(width: 12),
  ...
  Widget _uploadBtn(IconData icon, String label, Color color, VoidCallback onTap, {bool isRecording = false}) {
    return Expanded(
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: isRecording ? Colors.red.withOpacity(0.1) : color.withOpacity(0.1),
            borderRadius: BorderRadius.circular(24),
            border: Border.all(color: isRecording ? Colors.red : color.withOpacity(0.3), width: 1.5),
          ),
          child: Column(
            children: [
              Icon(isRecording ? Icons.stop_circle_rounded : icon, color: isRecording ? Colors.red : color, size: 32),
              const SizedBox(height: 8),
              Text(label, style: TextStyle(color: isRecording ? Colors.red : color, fontWeight: FontWeight.bold, fontSize: 13)),
            ],
          ),
        ),
      ),
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              Text(_uploadStatus, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1e293b))),
              const SizedBox(height: 8),
              Text('${(_uploadProgress * 100).toInt()}% اكتمل', style: const TextStyle(color: Color(0xFF64748b))),
            ],
          ),
        ),
      ),
