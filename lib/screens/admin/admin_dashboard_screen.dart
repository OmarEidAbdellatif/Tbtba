...
class _AdminDashboardScreenState extends ConsumerState<AdminDashboardScreen> with TickerProviderStateMixin { // حالة الشاشة مع دعم الأنيميشن
  ...
  void initState() { // دالة التهيئة الأولية عند تشغيل الشاشة
    ...
    _popController = AnimationController(vsync: this, duration: const Duration(milliseconds: 800))..forward();

    _fadeAnimations = List.generate(15, (index) { // إنشاء تسلسل حركات ظهور للعناصر (Staggered Animation)
          ...
          Expanded( // استهلاك المساحة المتبقية لعرض الشاشة المختارة
            child: IndexedStack( // تكديس الشاشات وعرض واحدة فقط بناءً على الفهرس
              ...
              children: [
                ...
                VisitApprovalView(fadeAnimations: _fadeAnimations), // شاشة مراجعة الزيارات
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildNavItem(2, Icons.calendar_month_outlined, 'الزيارات'), // تبويب الزيارات
          _buildNavItem(3, Icons.error_outline_rounded, 'الشكاوى'), // تبويب الشكاوى
          _buildNavItem(4, Icons.badge_outlined, 'الموظفين'), // تبويب الموظفين
          _buildNavItem(5, Icons.account_balance_outlined, 'التقارير'), // تبويب التقارير المالية
        ],
      ),
    );
  }
